...
addCapabilityToChannel() {
        ...
        if [ $CH_NAME != $SYS_CHANNEL ] ; then
                if [ $GROUP == "orderer" ]; then
                      ...
                      # Sign with PeerParents.Admin
                      signConfigtxAsPeerOrg 1 config_update_in_envelope.pb
                      # Sign with PeerHospital.Admin
                      ...
                      # Sign with PeerOParents.Admin
                      signConfigtxAsPeerOrg 1 config_update_in_envelope.pb
                      # Prepare to sign the update as the PeeHospital.Admin
                      setGlobals 0 2
                fi
        else
        ...
        if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
                peer channel update -f config_update_in_envelope.pb -c $CH_NAME -o orderer.poc-network.com:7050 --cafile $ORDERER_CA
                res=$?
                set +x
        else
                set -x
                peer channel update -f config_update_in_envelope.pb -c $CH_NAME -o orderer.poc-network.com:7050 --tls true --cafile $ORDERER_CA
                res=$?
                set +x
        fi
...
#Config update for /Channel on $CHANNEL_ALL
echo "Config update for /Channel on \"$CHANNEL_ALL\""
addCapabilityToChannel $CHANNEL_ALL channel

sleep $DELAY

#Config update for /Channel/Orderer
echo "Config update for /Channel/Orderer on \"$CHANNEL_ALL\""
addCapabilityToChannel $CHANNEL_ALL orderer

sleep $DELAY

#Config update for /Channel/Application
echo "Config update for /Channel/Application on \"$CHANNEL_ALL\""
addCapabilityToChannel $CHANNEL_ALL application

sleep $DELAY

#Query on chaincode on Peer0/Parents
echo "Querying chaincode on parents/peer0..."
chaincodeQuery 0 1 90

sleep $DELAY

#Invoke on chaincode on Peer0/Parents
echo "Sending invoke transaction on parents/peer0..."
